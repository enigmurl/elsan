...
if __name__ == '__main__':
    ...
    goptimizer = torch.optim.Adam(list(model.gparameters()), 1e-3, betas=(0.9, 0.999), weight_decay=1e-3)
    doptimizer = torch.optim.Adam(list(model.dparameters()), 1e-3, betas=(0.9, 0.999), weight_decay=1e-3)
    ...
    for i in range(start, O_MAX_EPOCH):
        ...
        model.train()
        lc = model.train_epoch(128, [goptimizer, doptimizer], max_out_frame=i+ 1)
